...
En tant que RELAIS, je notifie Emission suite à la réception d'un GeneralMessage en mode requête d'un producteur.
...
Dés que le formulaire s'affiche des "Paramètres" vérifiez que les champs suivants sont bien renseignés :
	...
	10- Partie "Services Producteurs" :
		1- Cocher "CheckStatus" et renseigner la valeur "http://localhost:8081/checkstatus";
		2- Cocher "GeneralMessage" et renseigner la valeur "http://localhost:8081/GeneralMessage";
...
- 9) Lancer "SNCF-test166" qui se trouve sous le projet "TEST_IVTR_UC_11_notif" mais en séléctionnant "NotifyGeneralMessage" correspondant au champ "Operation"

- 10) Importer le projet "DEV_STORY_753/soapui/TEST-753-soapui-project.xml" au niveau de SoapUI.

- 11) Lancer le "SiriProducerRpcBinding MockService" du "CheckStatus"

- 12) Vérifier avant de lancer le fichier "SiriProducerRpcBinding MockService" du "GetGeneralMessage" que la valeur de la date "ValidUntilTime" du bloc "GeneralMessage"
est supérieur à la date du jour

- 13) Lancer le fichier "SiriProducerRpcBinding MockService" du "GetGeneralMessage" afin de récupérer le message via la méthode "getGeneralMessage" et 
stocker le GeneralMessage puis créer le message "GeneralMessageDelivery" et envoyer le message "GeneralMessageDelivery"

- 14) Vérififier au niveau "SNCF-test166" que la notification est reçu.